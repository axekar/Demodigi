...
This Python module implements a toy model from von Toussaint (2011) as
a way of demonstrating how different procedures for parameter fitting
give slightly different results.

In the model, we have a line segment of unit length extending from the
origin. For the sake of simplicity it is limited to being somewhere in
the first quadrant. n 'measurements' are generated by first selecting
points on the line and then adding some gaussian scatter. For
simplicity, the standard deviation of the scatter is taken to be known
in the fitting.

When trying to fit this line to the data, two different parametrisations
are used. One describes the line in terms of its angle alpha to the x-
axis. The other describes it in terms of the coefficient a in the
equation y = ax. When fitting, three procedures are used:
...
The module automatically generates data and does fitting as soon as an
experiment object is instantiated. The plot method also creates plots
of the most interesting results, including:

 - The simulated measurements
 - The likelihood for the gaussian scatter, for each measurement
 - The likelihood function for the parameters
 - The posterior function for the parameters
 - The fits to the data, for each method

It also generated gifs based on the plots of the likelihood of the
scatter and of the fits to the data.
