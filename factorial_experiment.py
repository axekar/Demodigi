...
class participant(ABC):
   ...
   def save_results(self, path):
      ...
      if self.results_read:
         ...
         f.write("ID: {}".format(self.ID))
         f.write("number of sessions: {}".format(self.n_sessions))
         f.write("number of skills tested: {}".format(self.n_skills))
         f.write("Results:\n")
         full_string = []
         for row in range(self.results):
            row_string = []
            for entry in row:
               row_string.append("{}".format(entry))
            full_string.append((row_string.join(", ")))
         f.write(full_string.join("\n"))
         f.close()
      else:
...
class simulated_participant(participant):
   ...
   def calculate_results(self, n_sessions, n_skills):
      """
      Simulate results on a learning module
      """
