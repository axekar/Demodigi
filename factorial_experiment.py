...
class participant(ABC):
   ...
   def save_results(self, folder_path):
      ...
      if self.results_read:
         if folder_path[-1] != '/':
            folder_path += '/'
         f = open("{}{}_results".format(folder_path, self.ID), 'w')
         f.write("# File generated by demodigi.py\n")
         f.write("ID:\n")
         f.write("{}\n".format(self.ID))
         f.write("number of sessions:\n")
         f.write("{}\n".format(self.n_sessions))
         f.write("number of skills tested:\n")
         f.write("{}\n".format(self.n_skills))
         f.write("Results:\n")
         full_string = []
         for row in self.results:
            row_string = []
            for entry in row:
               row_string.append("{}".format(entry))
            full_string.append((", ".join(row_string)))
         f.write("\n".join(full_string))
         f.close()
      else:
   ...
   def load_results(self, folder_path):
      ...
      f = open(folder_path + self.ID + '_results', 'r')
      lines = f.readlines()
      f.close()
      lines = _trim_comments(lines)
      
      # This is really ugly, and I plan to replace it with JSON, once I get
      # around to it
      try:
         start_ID = lines.index("ID:")
      except ValueError:
         print("File does not match expected format")
         return
      self.ID = lines[start_ID + 1]
      
      try:
         start_sessions = lines.index("number of sessions:")
      except ValueError:
         print("File does not match expected format")
         return
      self.n_sessions = int(lines[start_sessions + 1])
      
      try:
         start_skills = lines.index("number of skills tested:")
      except ValueError:
         print("File does not match expected format")
         return
      self.n_skills = int(lines[start_skills + 1])
      
      try:
         start_results = lines.index("Results:")
      except ValueError:
         print("File does not match expected format")
         return
      self.results = np.zeros((self.n_skills, self.n_sessions), dtype = bool)
      for i in range(self.n_skills):
         line = lines[start_results + 1 + i].split(',')
         for j in range(self.n_sessions):
            self.results[i, j] = line[j].strip() == 'True'
