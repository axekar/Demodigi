   ...
   """
    ...
    def save_factorial_experiment_data(self, folder_path):
      ...
      """
        ...
        packed = json.dumps(
            {'ID': self.ID, 'Number of sessions': self.n_sessions(), 'Number of skills tested': self.n_skills(),
    ...
    def plot_results_by_time(self, folder_path):
      ...
      """
        ...
        sorted_dates_accumulated = list(
            zip(*sorted(zip(self.accumulated_by_date.keys(), self.accumulated_by_date.values()))))
...
class learning_module:
   ...
   """
    ...
    def __init__(self, competencies, n_sessions=np.nan, participants=None, start_date=_effective_min_date,
                 end_date=_effective_max_date, section_slug=None):
        """
      Parameters
      ----------
    ...
    def import_raw_analytics(self, filepath):
      ...
      """
        ...
        if self.section_slug is not None:
            cleaned = cleaned[cleaned['Section Slug'] == self.section_slug]

        self.raw_data_full = pd.DataFrame(
            data={'Student ID': cleaned['Student ID'], 'Date Created': cleaned['Date Created'],
                  'Activity Title': cleaned['Activity Title'], 'Attempt Number': cleaned['Attempt Number'],
                  'Correct?': cleaned['Correct?']})
        self.raw_data = self.raw_data_full[(self.start_date < self.raw_data_full['Date Created']) & (
                    self.raw_data_full['Date Created'] < self.end_date)].reset_index()
    ...
    def import_datashop(self, filepaths, verbose=False):
      ...
      """
        ...
        for filepath in filepaths:
            ...
            for child in root:
                if child.tag == 'context_message':
                    ...
                    try:
                        full_problem_name = \
                        child.findall('dataset')[0].findall('level')[0].findall('level')[0].findall('problem')[
                            0].findall('name')[0].text
                    except IndexError:
        ...
        self.xml_dataframe_full = pd.DataFrame(
            data={'Student ID (lowercase)': IDs, 'Date Created': answer_dates, 'Next Date': next_dates,
                  'Activity Title': activity_titles, 'Correct?': correct})
        self.xml_dataframe = self.xml_dataframe_full[(self.start_date < self.xml_dataframe_full['Date Created']) & (
                    self.xml_dataframe_full['Date Created'] < self.end_date)].reset_index()
        self.results_read = True
        return
    ...
    def _infer_mapping_pseudonym_ID(self, verbose=True):
      ...
      *IT DOES NOT WORK*, so it will need fixing if you want to use it.
      """
        ...
        mapping = {'Student ID (lowercase)': [], 'Pseudonym': [], 'Match reliable': [], 'Match percentage': [],
                   'n matches': [], 'Total occurrences': [], 'Second best match percentage': []}
            ...
            for pseudonym in pseudonyms:
                ...
                for time_span, problem_names in batches.items():
                    ...
                    for problem_name, batch_answers in problem_names.items():
                        ...
                        pseudonym_answers = pseudonym_entries[not_too_early & not_too_late & same_problem][
                            'Correct?'].values
        ...
        if verbose:
            ...
            print('Of these, {} could be reliably matched to pseudonyms in the raw_analytics file'.format(
                len(mapping_lowercaseID_pseudonym)))
    ...
    def import_data(self, raw_analytics_path, xml_path, verbose=False, previous_mapping_path=None):
      ...
      """
        ...
        self.full_results = pd.DataFrame(
            data={'Student ID': student_ids, 'Date Created': date_created, 'Activity Title': activity_title,
                  'Attempt Number': attempt_number, 'Correct?': correct})
        return
    ...
    def _setup_result_reading(self, participant):
      ...
      """
        ...
        participant.answer_date = pd.DataFrame(columns=self.skills, index=range(1, self.n_sessions + 1),
                                               dtype='datetime64[s]')
        participant.correct_first_try = pd.DataFrame(columns=self.skills, index=range(1, self.n_sessions + 1),
                                                     dtype=bool)
      ...
      """
        ...
        for skill in self.skills:
            for session in range(1, self.n_sessions + 1):
                try:
                    correct_skill = correct_participant[
                        correct_participant['Activity Title'] == '{}_Q{}'.format(skill, session)]
    ...
    def _read_participant_results_from_xml(self, participant):
      ...
      """
        ...
        self._setup_result_reading(participant)
        correct_participant = self.xml_dataframe[
            self.xml_dataframe['Student ID (lowercase)'] == participant.ID.lower() + '@arbetsformedlingen.se']
        n_answers = 0
        for skill in self.skills:
            for session in range(1, self.n_sessions + 1):
                correct_skill = correct_participant[
    ...
    def read_participants_results(self, database='combined', verbose=True):
      ...
      """
        ...
        if (database == 'combined' and type(self.full_results) == NoneType) or (
                database == 'datashop' and type(self.xml_dataframe) == NoneType):
