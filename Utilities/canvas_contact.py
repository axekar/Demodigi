...
Arbetsförmedlingen, and by extension also jobseekers. This will be
...
def account_name_user_id_mapping(token, verbose=False):
   ...
   """
    ...
    def find_next_link(link_long_string):
        """
      The get requests return a string which can be broken up into a comma-separated list,
      where each entry is a link to the first, current, next and previous (where a next and previous exist).
      This picks out which one is the next.
      """
    ...
    if not 'OK' in response.headers['Status']:
        raise UnexpectedResponseError(
            'When accessing user list, canvas returned status "{}"'.format(response.headers['Status']))
...
def send_file_contents(file_path, user_id, subject, token):
   ...
   """
    ...
    if not type(response_content) == list:
        raise UnexpectedResponseError(
            'When uploading, canvas returned error message "{}"'.format(response_content['errors'][0]['message']))
    return
...
def upload_file(file_path, canvas_path, user_id, token):
   ...
   """
    ...
    if not 'OK' in response_1.headers['Status']:
        raise UnexpectedResponseError(
            'When preparing for upload, canvas returned status "{}"'.format(response_1.headers['Status']))
    ...
    if not response_2_content['upload_status'] == 'success':
        raise UnexpectedResponseError(
            'When uploading, canvas returned status "{}"'.format(response_2.headers['Status']))
...
def upload_conversation_attachment(file_path, user_id, token):
   ...
   """
    ...
    response_1 = r.get('https://af.instructure.com/api/v1/users/{}/folders'.format(user_id),
                       headers={'Authorization': 'Bearer {}'.format(token)})
    if not 'OK' in response_1.headers['Status']:
        raise UnexpectedResponseError(
            'When locating conversation attachment folder, canvas returned status "{}"'.format(
                response_1.headers['Status']))
    response_1_content = response_1.json()
    ...
    file_id = upload_file(file_path, 'https://af.instructure.com/api/v1/folders/{}/files'.format(folder_id), user_id,
                          token)
    return file_id
...
def read_account_names(filepath):
   ...
   """
    ...
    if file_ending == 'txt':
        f = open(filepath)
        # I don't think this replacement is necessary anymore.
...
def send_feedback(account_name_path, feedback_folder_path, self_account, subject, message, token, verbose=False,
                  test=False):
    """
   ...
   """
    ...
    def update_trackers(already_received_feedback, received_feedback_now, daily_tracker_file_path,
                        total_tracker_file_path, tracker_memory_file_path):
    ...
    for target_account in tqdm.tqdm(accounts):
        file_path = '{0}{1}/Återkoppling_deltagare_{1}.docx'.format(feedback_folder_path,
                                                                    target_account.replace('/', '_'))
            ...
            if target_account in already_received_feedback:
                ...
                try:
                    if not test:
                        ...
                        if n_sent % 100 == 0:
                            update_trackers(already_received_feedback, received_feedback_now, daily_tracker_file_path,
                                            total_tracker_file_path, tracker_memory_file_path)
                    n_sent += 1
                except KeyError:
    ...
    update_trackers(already_received_feedback, received_feedback_now, daily_tracker_file_path, total_tracker_file_path,
                    tracker_memory_file_path)
